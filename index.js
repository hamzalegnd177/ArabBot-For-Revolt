  ...
  // 📝 ID قناة اللوج
  const LOG_CHANNEL_ID = "01K4T3884S43RW4SRV330H4C8F"; // حط هنا ID القناة بتاعة اللوج

  // --- Express web server علشان UptimeRobot يعمله ping ---
  const app = express();
  ...
  if (!token) {
    console.error("❌ مفيش TOKEN موجود في الإعدادات! روح اضيفه في Secrets");
    process.exit(1);
  }

  // login داخل IIFE عشان نتعامل مع الawait بأمان
  (async () => {
    try {
  ...
  if (command === "help") {
    await message.channel?.sendMessage({
      embeds: [{
        title: "📜 قائمة الأوامر",
        description:
          "استخدم الأوامر دي عشان تتعامل مع البوت 👇\n\n" +
          "🏓 **!ping** → اختبار البوت\n" +
          ...
          "💰 **!credits** → عرض رصيدك (وهمي)\n" +
          "🎁 **!daily** → خد مكافأة يومية (وهمية)\n" +
          "😂 **!wr3** → البوت يقول ورع 😂🔥\n" +
          "🔨 **!kick @user** → طرد عضو\n" +
          "🔨 **!ban @user** → حظر عضو\n" +
        ...
        colour: "#5865F2",
        footer: {
          text: "ArabBot v1.0 | مطور: Hamza177"
        }
      }]
    });
  }
